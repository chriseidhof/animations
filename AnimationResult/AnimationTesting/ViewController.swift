...
struct Animation<A> {
    let value: (RelativeTime) -> AnimationResult<A>
    
    init(_ value: @escaping (RelativeTime) -> AnimationResult<A>) {
        self.value = value
    }
    
    init(constant: A) {
        self.value = { _ in .done(constant, at: 0) }
    }
...
extension Animation {
    func parallel<B, C>(_ other: Animation<B>, _ transform: @escaping (A,B) -> C) -> Animation<C> {
        return Animation<C> { time in
            ...
            return a.zip(b, with: transform)
        }
    }
    
    func sequential<B>(_ other: @escaping (A) -> Animation<B>) -> Animation<Either<A,B>> {
        return Animation<Either<A,B>> { time in
            let a = self.value(time)
            switch a {
            case let .done(value, finishTime):
                return other(value).value(time-finishTime).map(Either.right)
            case .inProgress:
                return a.map(Either.left)
            }
...
func *<A,B>(lhs: Animation<A>, rhs: Animation<B>) -> Animation<Either<A,B>> {
    return lhs.sequential { _ in rhs }
}
...
extension Animation {
    func map<B>(_ f: @escaping (A) -> B) -> Animation<B> {
        return Animation<B> { self.value($0).map(f) }
    }
    
    func mapWithTime<B>(_ f: @escaping (RelativeTime, A) -> B) -> Animation<B> {
        return Animation<B> { time in
            return self.value(time).map { f(time,$0) }
        }
    }
    
    func stop(after endTime: RelativeTime) -> Animation<A> {
        return Animation { time in
            if time >= endTime {
                return .done(self.value(endTime).value, at: endTime)
            } else {
...
extension Animation where A == CGFloat {
    ...
    init(from: CGFloat, to: CGFloat, duration: TimeInterval, curve: @escaping AnimationCurve = linear) {
        ...
        self = Animation(inProgress: from).addSpeed(speed).stop(after: duration).augmentCurve(curve: curve, duration: duration).stop(after: duration) // the double stop isn't nice :(
    }
}
...
extension Animation {
    ...
    func delay(by: RelativeTime, initialValue: A) -> Animation {
        return Animation(inProgress: initialValue).stop(after: by) * self
    }

    fileprivate func augmentCurve(curve: @escaping AnimationCurve, duration: RelativeTime = 1.0) -> Animation {
        return Animation { time in
            self.value(curve(time/duration))
        }
    }
}
...
struct InterpretedAnimation {
    ...
    init<A>(animation: Animation<A>, interpret: @escaping (A) -> ()) {
        run = {
            ...
            if case .done = result { return .done }
            return .running
        }
    }
}
    ...
    override func viewDidAppear(_ animated: Bool) {
        ...
        func makeAnimation(from: CGFloat, to: CGFloat) -> Animation<CGFloat> {
            return Animation(from: from,
                             to: from + (to-from)*1.2,
                             duration: 0.37,
                             curve: builtin(.easeIn)).sequential { previous in
                Animation(from: previous, to: to, duration: 0.63, curve: builtin(.easeOut))
            }.map { $0.value }
        }
